@doc raw"""
    neutrals_normal(R̲̲, R̲̲⁽ⁿ⁾, n̲ₜ; s_pop_prior, σ_pop_prior, λ_prior)
...
- `logσ_pop_prior::VecOrMat{Float64}=[0.0, 1.0]`: Vector or Matrix with the
    ...
    rows in the matrix as pairs of time adjacent time points in dataset.
- `logλ_prior::VecOrMat{Float64}=[3.0, 3.0]`: Vector or Matrix with the
    ...
    If `typeof(logλ_prior) <: Matrix`, there should be as many rows in the
    matrix as number of barcodes × number of time points in the dataset.
"""
Turing.@model function neutrals_normal(
    R̲̲::Matrix{Int64},
    n̲ₜ::Vector{Int64},
    n_neutral::Int,
    n_bc::Int=1;
