...
function mcmc_joint_fitness_group(;
    ...
    # Define index of keys in group
    group_idx = [x ∈ group for x in data_keys]
    ...
    # Select mutant matrix with the group elements.
    R̲̲⁽ᵐ⁾ = R_mut[:, group_idx]

    # Build vector with sum of barcode elements not part of the group
    R̲̲⁽ᴹ⁾ = sum(R_mut[:, .!group_idx], dims=2)
    ## %%%%%%%%%%% Total barcodes data %%%%%%%%%%% ##

    # Concatenate neutral and mutant data matrices
    R̲̲ = hcat(R̲̲⁽ⁿ⁾, R̲̲⁽ᵐ⁾, R̲̲⁽ᴹ⁾)
...
function mcmc_joint_mean_fitness(;
    ...
    # Sample posterior
    chain = Turing.sample(
        model(R̲̲⁽ⁿ⁾, R̲̲, n̲ₜ; model_kwargs...),
        sampler,
        ensemble,
        n_steps,
        n_walkers,
